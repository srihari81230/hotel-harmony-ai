      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            Adjust the price slider above — we'll recommend hotels in your range, lowest price first.
          </p>
        </div>

        <div className="flex items-center justify-center gap-2 mb-10 flex-wrap">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${activeCategory === cat ? "bg-accent text-accent-foreground shadow-gold" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
              {cat}
            </button>
          ))}
        </div>
        ...
        <p className="text-center text-sm text-muted-foreground mb-6">
          {filtered.length} hotels found between ₹{priceRange[0].toLocaleString("en-IN")} and ₹{priceRange[1].toLocaleString("en-IN")}
        </p>
